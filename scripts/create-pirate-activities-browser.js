// Browser Console Script to Create 20 Pirate Activities
// Copy and paste this entire script into your browser console while logged into the app
// Make sure you're on a page where React is loaded (like the Dashboard)

(async function createPirateActivities() {
  console.log('🏴‍☠️ Starting to create pirate activities...');
  
  // Import the activitiesApi (this will work if you're in the app)
  // For browser console, we'll need to access it differently
...
- Reception: children help create the story structure.`,
  ...
  try {
    // Try to access React context through window or find the DataContext
    // This is a workaround for browser console execution
    console.log('📝 Preparing to create', pirateActivities.length, 'activities...');
    
    // Check if we can access the activitiesApi
    const modules = window.__REACT_DEVTOOLS_GLOBAL_HOOK__?.renderers?.get(1)?.currentDispatcherRef?.current;
    
    console.log('💡 Instructions:');
    console.log('1. Open the Activity Library in your app');
    console.log('2. Click "Create Activity"');
    console.log('3. Copy and paste each activity data from the array above');
    console.log('4. Or use the browser console to access the DataContext');
    
    // Store activities in window for easy access
    window.pirateActivities = pirateActivities;
    console.log('✅ Pirate activities stored in window.pirateActivities');
    console.log('📋 You can access them with: window.pirateActivities');
    
    return pirateActivities;
  } catch (error) {
    console.error('❌ Error:', error);
    console.log('💡 Activities are stored in window.pirateActivities');
    return pirateActivities;
  }
})();
